...
for i in *.dedup.bam;
do SAMPLE=${i%%.*};
gatk --java-options "-XX:ConcGCThreads=1" HaplotypeCaller -R $REF -I ${SAMPLE}.dedup.bam -ERC GVCF --do-not-run-physical-phasing -O ${SAMPLE}.dedup.GATK_HC.g.vcf -L $TARGETS_CALLING;
done
...
vep --cache --offline --fork 24 --species homo_sapiens --refseq \
	--assembly $GRCh -i raw_variants.norm.vcf.gz -o vep_local_output.vcf \
